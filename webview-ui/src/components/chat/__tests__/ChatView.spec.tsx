...
const _mockVersionIndicator = vi.mocked(
...
describe("ChatView - Version Indicator Tests", () => {
	...
	it("does not display version indicator for SyntX provider", () => {
		const { queryByTestId, queryByLabelText } = renderChatView()

		// First hydrate state with no messages (welcome screen)
		mockPostMessage({
			clineMessages: [],
		})

		// For SyntX provider, version indicator should not be displayed
		const versionButtonByTestId = queryByTestId("version-indicator")
		const versionButtonByLabel = queryByLabelText(/version/i)

		expect(versionButtonByTestId).not.toBeInTheDocument()
		expect(versionButtonByLabel).not.toBeInTheDocument()
	})
