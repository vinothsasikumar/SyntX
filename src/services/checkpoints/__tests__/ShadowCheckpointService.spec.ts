...
const initWorkspaceRepo = async ({
	workspaceDir,
	userName = "Syntx",
	userEmail = "support@orangecat.ai",
	...
	(klass, prefix) => {
		...
		describe(`${klass.name}#hasNestedGitRepositories`, () => {
			it("throws error when nested git repositories are detected during initialization", async () => {
				...
				await mainGit.init()
				await mainGit.addConfig("user.name", "Syntx")
				await mainGit.addConfig("user.email", "support@orangecat.ai")
				...
				await nestedGit.addConfig("user.name", "Syntx")
				await nestedGit.addConfig("user.email", "support@orangecat.ai")
